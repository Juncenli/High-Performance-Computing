...
def load():
    ...
    train_set = torchvision.datasets.CIFAR10(root='./cifar10-dataset', train=True,
                                             download=True, transform=transform)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=128,
                                               shuffle=True, num_workers=2)

    test_set = torchvision.datasets.CIFAR10(root='./cifar10-dataset', train=False,
                                            download=True, transform=transform)
    ...
    for epoch in range(total_epoch):
        ...
        if best_test_acc < float(test_accuracy):
            ...
            if not os.path.isdir('./model'):
                os.mkdir('./model')
            torch.save(net.state_dict(), "./model/cifar10_googlenet_CNN.ckpt")
    
    end = time.time();
    print("The training time of cifar10 googlenet pytorch is:", end-start,"(seconds)")
    print('Model saved in file: ./model/cifar10_googlenet_CNN.ckpt')
