...
void print_state(int thread_num, string state, string action) {
    if (!all_philosophers_done()) { // check if all the philosophers are done or not, or else we output bad data
        ...
        cout << "- " << action << " left fork: No." << get_left_fork(thread_num) << endl;
        cout << "- " << action << " right fork: No." << get_right_fork(thread_num) << endl << endl;
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        ...
        cout << "Error: You need to give an input <number of philosophers>" << endl;
        return 1;
    }
